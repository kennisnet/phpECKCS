...
class EckResponseNormalizerTest extends TestCase
{
    ...
    public function testRecordNormalizerFromSearchResponse()
    {
        ...
        $this->assertTrue($normalizer->supportsSchema(EckRecordSchemaTypes::ECKCS_2_5_2));
        $this->assertFalse($normalizer->supportsSchema(EckRecordSchemaTypes::ECKCS_2_5));
        ...
        $records = $normalizer->normalize($deserializedRecords, EckRecordSchemaTypes::ECKCS_2_5_2);
        $this->assertEquals(10, count($records));
        $this->assertInstanceOf(EckRecord::class, array_shift($records));
    }
